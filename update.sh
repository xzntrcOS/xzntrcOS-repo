...
declare -a OLD_DATABASE=(
						 ...
						 'xzntrcos-repo.db'
						 'xzntrcos-repo.db.tar.gz'
						 'xzntrcos-repo.db.tar.gz.old'
						 'xzntrcos-repo.files'
						 'xzntrcos-repo.files.tar.gz'
						 'xzntrcos-repo.files.tar.gz.old'
...
repo-add xzntrcos-repo.db.tar.gz *.pkg.tar.zst
echo -e "\n${BGreen}[+] Successful Updating ✔"
sleep 0.5

# remove syslink to avoid any issues #
echo -e "${BRed}\n[+] Removing syslink to avoid any Issues...\n${Color_Off}"
rm xzntrcos-repo.db xzntrcos-repo.files
mv xzntrcos-repo.db.tar.gz xzntrcos-repo.db
mv xzntrcos-repo.files.tar.gz xzntrcos-repo.files
echo -e "\n${BGreen}[+] Successful Removing ✔"
sleep 0.5
